...
### 4.1 원문 링크(딥링크) 형식 — 2026-08-20 실측 정정

**정정 전 기술(오류):** `…w2xPath=/pgj/ui/pgj100/PGJ15AF01.xml`을 "사건상세 프레임" 딥링크로 적었다. 근거는 `GET 200`뿐이었고 화면을 렌더해 보지 않았다(당시 브라우저 팬이 이 SPA를 못 띄웠다 — AGENTS §9 2026-07-18). 그 URL로 들어가면 **오류창 "부모 객체를 찾을 수 없습니다."** 가 뜨고, 닫아도 법원·사건번호가 빈 화면에 "해당 사건번호는 잘못된 번호입니다."가 남는다.

- **화면 ID 규칙(구조).** `…F00`/`…M00` = 최상단 컨테이너 — `scwin.isMain → true`이고 헤더·본문 프레임을 스스로 구성하므로 **단독 진입이 된다.** `…F01`/`…M01` = 자식 화면 — `onpageload`가 `$p.getParameter("param")`으로 부모에게서 `csNo`·`cortOfcCd`를 받고 최상단 전역(`scglblo`)을 참조하므로 **단독 진입이 불가하다.**
- **사용할 링크:** `https://www.courtauction.go.kr/pgj/index.on?w2xPath=/pgj/ui/pgj100/PGJ159M00.xml` (경매사건검색). 값은 `src/lib/court-origin.ts` 한곳에서 정하고 `scripts/crawl-config.ts`가 재수출한다.
- **딥링크 부재 확증(2026-08-20):** 검색을 실행해도 URL이 바뀌지 않고(SPA), 조회는 POST(`/pgj/pgj15A/selectAuctnCsSrchRslt.on`)이며, 인쇄·문서뷰어는 세션 팝업이다. URL 파라미터로 사건을 주입할 경로는 없다.
- **도달 실측:** 법원(선택)·연도(선택)·번호(입력) 3칸을 채우면 사건내역이 그 화면에 렌더된다. 신규 컨텍스트(쿠키 0)·모바일 390px에서 15건 조회 중 11건 도달, 실패 4건은 전부 **데이터의 법원명 오류**(§9)이며 링크·표기 문제가 아니다.
- **입력칸 규격:** 번호 입력칸은 `maxlength="7"`에 한글을 지운다 — `2023타경104819`를 붙여넣으면 `20231`만 남는다. 그래서 앱의 복사값은 **번호 단독**이다.
- **법원 표기 차이:** 사이트 선택 목록은 지원명만 쓰거나(`천안지원`) 본원을 붙여 한 낱말로 쓴다(`부산서부지원`). 우리 표기(`대전지방법원 천안지원`)와 어긋나는 10종은 `src/lib/court-origin.ts`의 표로 정규화한다(실물 60종 스냅샷 = `tests/unit/court-select-options.ts`).
