  ...
  return (
    <>
      ...
      {/* 데이터 범위 기준(산출물 기준 공개) — 갱신 주기·수집 대상은 위 데이터 안내가 이미 말한다:
          여기는 건수 상한·배분 창·정렬 기준만 더한다(1정보 1표시). 배분 창은 갱신 주기와 별개의
          상수(OUTPUT_WINDOW_DAYS 7일)라 갱신이 매일로 바뀌어도 그대로다 — 갱신 주기에 기대어
          적으면 주기를 바꾼 날 이 문장이 거짓이 된다. 파일 소유권이 src/app/guide/** 한정이라
          GlossarySheet(픽 기준 공개 섹션) 내부가 아닌 페이지 말미에 둔다. */}
      <p className="px-4 text-[13px] leading-snug text-ink/75">
        수집일부터 7일간의 매각기일에 나누어 최대 1,000건을 제공하며, 같은 기일에서는 감정가
