...
function ListContent() {
  ...
  const apply = (next: ListQuery) => {
    window.history.replaceState(null, "", `/list${buildListQuery(next)}`);
  };
  const update = (patch: Partial<ListQuery>) => apply({ ...query, ...patch });

  // 필터 축 변경 전용 경로(감사 2차 55) — URL과 홈 세션 미러를 함께 갱신한다. 미러를 갱신하지 않으면
  // 여기서 해제한 조건이 뒤로가기 홈 복귀에서 되살아나 홈·리스트가 서로 다른 건수를 주장한다.
  // 표시 건수는 조건이 바뀔 때만 처음으로 되돌린다(정렬 변경은 유지 — 감사 2차 57).
  const updateFilters = (patch: Partial<ListQuery>) => {
    const next = { ...query, ...patch, count: 10 };
    apply(next);
  ...
  // 픽 전용 모드도 같은 줄의 칩으로 다룬다(감사 2차 56) — 해제 수단을 주고, 기준 안내 배너를
  // 따로 두지 않아 같은 조건을 두 번 표시하지 않는다(픽 기준은 홈 진입 카드·안내에 공개).
  const chips: { key: string; label: string; onRemove: () => void }[] = [
      ...
      ? [
          {
            key: "pick",
            label: "미친가치 픽",
            onRemove: () => updateFilters({ pickOnly: false }),
          },
        ]
      : []),
    ...query.regions.map((r) => ({
    ...
    ...query.districts.map((d) => {
      ...
      return {
        ...
        onRemove: () => updateFilters({ districts: query.districts.filter((x) => x !== d) }),
      };
    }),
    ...query.priceBands.map((b) => ({
      key: `b:${b}`,
      label: PRICE_BANDS.find((band) => band.key === b)?.label ?? b,
      onRemove: () => updateFilters({ priceBands: query.priceBands.filter((x) => x !== b) }),
    })),
    ...query.categories.map((c) => ({
      key: `c:${c}`,
      label: c,
      onRemove: () => updateFilters({ categories: query.categories.filter((x) => x !== c) }),
    })),
  ];
  ...
  const relaxActions = [
    ...(query.priceBands.length > 0
      ? [{ label: "금액 범위 넓히기", onClick: () => updateFilters({ priceBands: [] }) }]
      : []),
    ...(query.districts.length > 0
      ? [{ label: "시·군·구 조건 해제", onClick: () => updateFilters({ districts: [] }) }]
      : query.regions.length > 0
        ? [{ label: "지역 조건 해제(전국 보기)", onClick: () => updateFilters({ regions: [] }) }]
        : []),
    ...(query.categories.length > 0 &&
    query.priceBands.length === 0 &&
    query.districts.length === 0 &&
    query.regions.length === 0
      ? [{ label: "용도 조건 해제", onClick: () => updateFilters({ categories: [] }) }]
      : []),
  ];
  ...
  return (
    <>
      ...
      {/* 정렬은 같은 결과의 배열 순서만 바꾼다 — 펼친 표시 건수를 유지한다(감사 2차 57). */}
      <ItemList
        items={filtered}
        shown={query.count}
        sort={query.sort}
