...
- 데이터는 매일 03:00 KST 자동 갱신, 모든 화면에 데이터 기준일 표기
- 리프레쉬 탭: 새벽 정기 갱신을 기다리지 않고 지금 시점으로 다시 수집(경량 모드 실측 3분, 쿨다운 1시간)

## 아키텍처

```
[GitHub Actions — 매일 03:00 KST · 일 full(cron 0 18 * * 6 UTC) · 월~토 quick(0 18 * * 0-5 UTC)]
...
[리프레쉬 탭 → POST /api/refresh]
   └ 쿨다운(1h)·진행중 검사 → workflow_dispatch(mode=quick)
        └ 경량 수집: 매각기일 창 7일 + 상세 생략 (실측 182초·요청 162회)
