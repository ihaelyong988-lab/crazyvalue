...
### 세로 리듬 오버라이드 (2026-07-19 주인님 확정 — "세로폭 낭비 금지")
- 모바일 페이지 세로 간격: 섹션(카드) 간 **16px 기본**(`space-y-4`) — 20px 이상 금지(게이트 R11 warn).
- 안내·설명문(13px 보조 텍스트): **1줄 원칙** + `leading-snug`. 본문 15px 규정(line-height 1.5~1.7)은 읽기 본문에만 적용.
- 안내·상태 문구는 **1문장 압축**이 기본 — 「~습니다. ~습니다.」 병렬 2문장 금지(게이트 R12 warn). 압축 문체 예: 「홈 필터의 초기값 쓰임, 선택 즉시 이 기기에 저장.」 예외: 오류 화면 ErrorState의 message+action 2문장 규격(§13)·법적 고지·용어/데이터 읽기 본문.
- 페이지 하단 여백 = 하단 고정 요소 **실측 높이 + 8px**. 레이어(AppShell·페이지) 간 하단 패딩 중복 금지.
- 터치 타깃 ≥44px·결과 버튼 52px·칩 간 6px 등 접근성 플로어와 최소 간격은 세로 축소 대상에서 제외.
