...
/**
 * 산출물 배분 창(일) — 수집일(KST)부터 이 일수만큼의 매각기일에 OUTPUT_CAP건을 나눠 담는다.
 *
 * 창을 "다음 갱신일까지"로 잡았던 직전 설계는 주 1회 갱신에서만 성립했다. 매일 갱신에서는
 * 다음 갱신이 내일이라 창이 하루로 붕괴하고, 1,000건이 전부 하루짜리 기일에 몰린다
 * (그 다음 날부터 전 물건이 기일 경과 — AGENTS.md §9 2026-08-02 결함과 같은 형태다).
 * 그래서 배분 창은 갱신 주기에서 떼어내 상수로 고정한다 — 갱신이 잦아져도 방문자가 보는
 * 산출물의 유효수명은 7일로 유지된다.
 */
export const OUTPUT_WINDOW_DAYS = 7;
